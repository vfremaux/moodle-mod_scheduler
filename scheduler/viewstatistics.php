<?php
    ...
    */
    ...
    foreach ($tabs as $tab) {
        ...
        $row[] = new tabobject($tabname, "view.php?what=viewstatistics&amp;id=$cm->id&amp;course=$scheduler->course&amp;view=".$tab, $tabname);
    }
    $tabrows[] = $row;

    print_tabs($tabrows, get_string($view, 'scheduler'));

    //display correct type of statistics by request

    $attendees = get_users_by_capability($context, 'mod/scheduler:appoint', 'u.id,u.lastname,u.firstname', 'u.lastname,u.firstname', '', '', $usergroups);
        ...
        if (!empty($attendees)) {
           	...
           	$sql = "
                ...
                GROUP BY
                    a.studentid
           	";
           	$allappointments = count_records_sql($sql);
           	...
           	$sql = "
                ...
                GROUP BY
                    a.studentid
           	";
           	if ($statrecords = get_records_sql($sql, $from, $limit)) {
        	...
        	scheduler_print_pagination($cm->id, $view, $offset, $from, $limit, $allappointments, 'viewstatistics');
           	print_table($table);
        	scheduler_print_pagination($cm->id, $view, $offset, $from, $limit, $allappointments, 'viewstatistics');
        } else {
