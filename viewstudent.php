<?php
    ...
    */
    ...
    if ($subaction != ''){
        include_once "viewstudent.controller.php"; 
    }
    ...
    $order = @$order;
    $row[] = new tabobject($tabname, "view.php?what=viewstudent&amp;id={$cm->id}&amp;studentid={$studentid}&amp;course={$scheduler->course}&amp;order={$order}&amp;view=appointments", $tabname);
    $tabname = get_string('comments', 'scheduler');
    $row[] = new tabobject($tabname, "view.php?what=viewstudent&amp;id={$cm->id}&amp;studentid={$studentid}&amp;course={$scheduler->course}&amp;order={$order}&amp;view=notes", $tabname);
    $tabrows[] = $row;
    ...
    if ($slots = get_records_sql($sql)) {
        ...
        if($order == 'DESC'){
            $orderlink = "<a href=\"view.php?what=viewstudent&amp;id=$cm->id&amp;studentid=".$studentid."&amp;course=$scheduler->course&amp;order=ASC&amp;view=$view\">";
        } else {
            $orderlink = "<a href=\"view.php?what=viewstudent&amp;id=$cm->id&amp;studentid=".$studentid."&amp;course=$scheduler->course&amp;order=DESC&amp;view=$view\">";
        }
        ...
        foreach($slots as $slot) {
            ...
            if ($view == 'appointments'){
                ...
                //display notes
                $onsubmitcall = ($usehtmleditor && @$CFG->defaulthtmleditor != 'tinymce') ? "javascript:document.forms['updatenote{$slot->id}'].onsubmit();" : '' ;
                ...
                $commenteditor .= "<input type=\"hidden\" name=\"view\" value=\"appointments\" />\n";
        ...
        if ($view == 'appointments'){
            ...
            echo "<input type=\"hidden\" name=\"view\" value=\"appointments\" />\n";
